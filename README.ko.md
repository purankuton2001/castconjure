...
이미 오리지널 캐릭터를 가진 창작자(우선은 일본어로 작업하는 창작자)와, 영어·한국어 등 해외 언어로 시청하는 팬을 위해 만들었습니다. 아이돌은 하나, 채팅은 전 세계, 사용 가능한 것은 나만의 오리지널 캐릭터뿐. 실사풍이든 애니풍이든 자유입니다. [Conjure Board](https://purankuton2001.github.io/conjure-board/)에서는 아이돌별로 창작자의 언어가 아닌 언어에서 온 반응의 비율을 볼 수 있습니다.

castconjure는 같은 요구사항 문서로 두 모델이 만든 두 앱 중 하나입니다. 다른 하나는 [AI OshiBloom](https://github.com/purankuton2001/ai-oshibloom)(스튜디오 + 연속 라이브, GPT-6 제작). 어느 쪽도 다른 쪽의 덤이 아닙니다. 비교는 Board에서.
...
페르소나당 1회: 대기 12개 ≈ $3, 알아챔 6개 ≈ $1.5, 참조 ≈ $0.15. 30초마다 반응 1개면 60분 ≈ $30. 세션 상한 기본 $20. fal 대시보드의 월 한도도 설정하세요. Turbo의 초당 단가는 fal 페이지에 없어서 견적은 H3 Max와 같다고 가정합니다 (청구서가 다르면 `.env`의 `PRICE_TURBO_480P`로 덮어쓰세요).

## 안전 설계

1. **나만의 오리지널 캐릭터만.** 얼굴은 앱 안에서만 생성되며 UI에 업로드는 없습니다 (`persona:import`는 직접 그린 그림을 넣는 개발자용 경로). 얼굴 생성 프롬프트에는 항상 "가상의 성인, 실존 아이돌이나 기존 캐릭터를 닮지 않음"이 붙습니다
...
MIT. 생성 비용은 *당신의* fal 계정에 직접 청구되며, 이 프로젝트는 지불하거나 중계하지 않습니다. 모델: MiniMax H3 via [fal](https://fal.ai); 생성된 미디어는 fal 및 MiniMax의 이용약관을 따릅니다.
