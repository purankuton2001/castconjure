...
右カラムの **Persona** セクション。UI は英語が既定で、`http://127.0.0.1:8787/?lang=ja` で日本語になる（overlay も `?lang=ja`）。同梱の白詰ゆいがあるので、まずはこれで通す。**Style** で photoreal / anime を選べる（顔ガチャと毎クリップのプロンプトに効く）。

1. **顔ガチャ**：「4 枚引く」→ 気に入った顔をクリック → 全身と世界観の参照画像を派生生成 → 3 枚を見て「確認して保存」。顔はここで生成したものしか使えない（写真アップロードはない）。FAL_KEY がなければプレースホルダ画像
2. **声**：EN / KO / JA のボタンで言語ごとに TTS 生成。実在人物の声は使わない。推しはコメントの言語で返し、その言語の声を参照に使う
3. **人格**：名前・ファンの呼び方・見た目（英語）・シグネチャ・既定の世界観・話し方・口癖・禁止事項・返事のシステムプロンプト。「人格を保存」
4. **待機映像プール**：本数を決めて「生成する」。推定費用が出る（fal で 12 本 ≈ $3、mock/local は $0）。できた分から overlay の待機層で自動ループ

推しはフォルダ `data/personas/<id>/` に入る。コピーすれば別マシンで同じ推し。

**反応クリップの方式**（Generation の Reaction clip mode）：既定は `i2v turbo`。待機ループの 1 フレームを起点に H3 Max Turbo で生成し、コメント → 画面が 4〜8 秒・1 本 $0.25。推しごとの**固定 seed**と毎回同じ声・見た目の説明文で、参照音声なしでも顔と声が揃う（Persona の Seed と Voice description）。声の一貫性を最優先するなら `r2v`（参照画像＋参照音声、約 10〜13 秒・$0.46）。
...
|---|---|---|---|
| H3 Max Turbo image-to-video（既定） | $0.05/秒 | $0.25 | 約 $30 |
| reference-to-video（参照 3 枚＋参照音声） | $0.08/秒 + $0.02/枚 | $0.46 | 約 $55 |
...
| `ip` | 実在アイドル・グループ・アニメ／ゲームのキャラと作品名、「〇〇に似せて」「〇〇の振付」（`data/ip-names.txt`、F-16）。返事・人格・顔ガチャの指示にも適用 |
...
fal 経路の実測は 4〜8 秒、mock は約 2.5 秒。
...
npm run probe:voice -- --langs en,ko,ja   台詞と参照音声の実測（言語ごと）
